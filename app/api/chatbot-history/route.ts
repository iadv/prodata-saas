...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = `
      ...
      FROM "${schemaName}"."chatbot_historical"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingPrompt.rows.length > 0) {
      const updateQuery = `
        UPDATE "${schemaName}"."chatbot_historical"
        SET created_at = NOW()
        WHERE id = $1
    ...
    const insertQuery = `
      INSERT INTO "${schemaName}"."chatbot_historical" (prompt)
      VALUES ($1)
      RETURNING id, prompt, created_at as "createdAt"
    `;
...
async function ensureHistoricalTableExists(schemaName: string) {
  try {
    ...
    const checkTableQuery = `
      ...
      AND table_name = 'chatbot_historical'
    `;
    ...
    if (tableExists.rows.length === 0) {
      const createTableQuery = `
        CREATE TABLE IF NOT EXISTS "${schemaName}"."chatbot_historical" (
          id SERIAL PRIMARY KEY,
          prompt TEXT NOT NULL,
          created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
        );
        
        CREATE INDEX IF NOT EXISTS idx_chatbot_historical_created_at 
        ON "${schemaName}"."chatbot_historical"(created_at DESC);
      `;
