...
export function ChatInterface({ selectedTables }: ChatInterfaceProps) {
  ...
  const handleGenerateChart = async (messageId: string) => {
    ...
    try {
      // Only generate the chart config without updating the database
      const response = await fetch('/api/chart', {
        ...
        body: JSON.stringify({
          ...
          // Don't pass messageId and conversationId to avoid the database update
        }),
      });
      ...
      // Update message with chart data in UI state
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === messageId
            ? {
      ...
      // Update the message in the database using a separate API endpoint
